...
| **자정 임박** | 오후 11시 30분이 되면 진행 중인 작업을 마무리하고 push한다 |

**자정 임박 트리거 운영 방식**

- 오후 11시 30분 기준으로 잡은 이유: 브리핑 작성 → README 최신화 → 커밋 → push 까지 여유 있게 완료하고 자정 이전에 하루치 작업을 확실히 저장하기 위함.
- 작업 도중이라면 현재 단계를 완성 가능한 최소 단위까지만 마무리한 뒤 push한다. 미완성 상태로 중단하지 않는다.
- push 후 남은 작업은 브리핑의 **⏭ 다음에 할 것** 항목에 기록한다.
...
```
1. 당일 작업 브리핑을 사용자에게 먼저 전달한다 (아래 브리핑 형식 적용)
2. git status 로 변경 파일 확인
3. .env 파일은 절대 staging 하지 않는다
4. README.md 를 현재 구현 상태에 맞게 최신화한다 (아래 규칙 적용)
5. 변경된 파일을 파일명 지정 방식으로 git add (README.md 포함)
6. 커밋 메시지 작성 (한국어, 변경 내용 요약)
7. git push https://github.com/chang-seon/my_porject.git main
8. Push 완료 여부를 사용자에게 보고한다
```

**당일 작업 브리핑 형식 (1번 단계 적용)**

push 전 아래 형식으로 당일 작업 내용을 한눈에 보기 쉽게 정리해 전달한다.
...
⏭ 다음에 할 것 (있는 경우)
  • 다음 세션에서 이어서 해야 할 작업
─────────────────────────────
위 내용으로 GitHub에 push하겠습니다.
```

- 기술 용어는 쉬운 말로 풀어서 설명한다.
- 항목이 없는 섹션(예: 수정 없으면 🔧 섹션)은 생략한다.
