...
- **모든 구현은 현업(실무) 수준의 방법을 기준으로 한다.**
- 업계에서 더 이상 권장하지 않는 구식 패턴(deprecated, legacy)은 사용하지 않는다.
- 현업에서 표준으로 자리잡은 새로운 방식이 있다면, 기존 코드도 해당 방식으로 교체한다.
- 적용 기준 예시:
  - Python: `dataclass` 대신 `pydantic v2`, `setup.py` 대신 `pyproject.toml` 등
  - 비동기: I/O 병목 구간은 `asyncio` / `ThreadPoolExecutor` 활용
  - 타입 힌팅: Python 3.10+ 네이티브 문법 우선 (`X | Y`, `list[str]` 등)
  - 테스트: `unittest` 대신 `pytest`
  - 환경 관리: `venv` + `requirements.txt` → 필요 시 `uv` 또는 `poetry` 전환 검토
- 방식 변경이 기존 코드에 영향을 줄 경우, 변경 전 사용자에게 먼저 알리고 승인을 받는다.
...
**Word 문서 최신화 기준 (README.md 최신화와 동시에 적용)**

Push 절차 4번 단계(README.md 최신화)와 동시에 아래 Word 문서도 함께 업데이트한다.

- **파일 경로**: `자율협업AI에이전트_프레임워크_보고서.docx` (프로젝트 루트)
- **생성 방법**: `_make_doc.py` 스크립트를 실행하여 재생성한다.
- **한글 깨짐 방지**: 모든 텍스트 run에 `맑은 고딕` 폰트를 명시적으로 설정한다 (East Asian 폰트 XML 속성 포함).
- **업데이트 트리거**: 새 기능 추가 / 구조 변경 / 규칙 변경이 있을 때마다 반드시 재생성한다.
- **내용 구성**: 프로젝트 개요, 에이전트 역할, 파일 구조, 실행 방법, 환경변수, 기술 스택, 향후 계획을 포함한다.
- `_make_doc.py`는 실행 후 삭제하지 않고 프로젝트 루트에 유지한다 (언제든 재생성 가능하도록).
